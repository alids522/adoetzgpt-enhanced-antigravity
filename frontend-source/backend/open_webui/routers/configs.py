...
@router.get('/gemini', response_model=GeminiConfigForm)
async def get_gemini_config(request: Request, user=Depends(get_admin_user)):
    return {
        'GEMINI_API_KEY': request.app.state.config.GEMINI_API_KEY,
        'GEMINI_API_BASE_URL': request.app.state.config.GEMINI_API_BASE_URL,
    }


@router.post('/gemini', response_model=GeminiConfigForm)

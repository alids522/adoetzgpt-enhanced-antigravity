<script lang="ts">
	...
	import TokenUsageDashboard from './TokenUsageDashboard.svelte';

	const BREAKPOINT = 768;
	const DEFAULT_PINNED_ITEMS = ['notes', 'workspace', 'tokenUsage'];
	...
	const getMenuItemMeta = (id) => {
		const items = {
			...
			tokenUsage: { label: 'Token Usage', href: '#token-usage', iconType: 'tokenUsage' }
		};
		return items[id];
	};
	...
	const tokenUsageClickHandler = async (event?: Event) => {
		event?.stopImmediatePropagation();
		event?.preventDefault();
		showTokenUsageDashboard.set(true);
							...
							<Tooltip content={$i18n.t(meta.label)} placement="right">
								<a
									...
									on:click={async (e) => {
										e.stopImmediatePropagation();
										e.preventDefault();
										if (itemId === 'tokenUsage') {
											await tokenUsageClickHandler(e);
										} else {
											goto(meta.href);
											itemClickHandler();
										}
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									<a
										...
										href={meta.href}
										on:click={itemId === 'tokenUsage' ? tokenUsageClickHandler : itemClickHandler}
										draggable="false"
										aria-label={$i18n.t(meta.label)}
									>
										<div class="self-center">

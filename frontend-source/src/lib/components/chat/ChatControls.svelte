...
<script lang="ts">
	...
	const getGeminiLiveContext = () => {
		const messages = history?.currentId ? createMessagesList(history, history.currentId) : [];
			...
			.map((message) => {
				const speaker =
					message.role === 'assistant'
						? (message.modelName ?? message.model ?? 'assistant')
						: message.role;
	...
	$: specialPanel = $showCallOverlay || $showGeminiLiveOverlay || $showArtifacts || $showEmbeds;
</script>

{#if !largeScreen}
	{#if $showCallOverlay}
		<div
			class="fixed inset-0 z-[999] bg-white text-gray-700 dark:bg-black dark:text-gray-300 flex justify-center"
		>
			<CallOverlay
				bind:files
				{submitPrompt}
	...
	{#if $showGeminiLiveOverlay}
		...
		>
			<GeminiLiveOverlay
				...
				{modelId}
				{eventTarget}
				conversationContext={getGeminiLiveContext()}
				onMessageAdd={handleGeminiMessage}
				on:close={() => {
	...
	>
		{#if $showControls}
			<div class="flex max-h-full min-h-full">
				<div
					class="w-full {specialPanel && !$showCallOverlay && !$showGeminiLiveOverlay
						? ' '

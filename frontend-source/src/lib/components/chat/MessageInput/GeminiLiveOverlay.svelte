<script lang="ts">
	...
	let nativeLiveVoiceStarted = false;
	...
	const getAdoetzNative = () => (globalThis as any).Capacitor?.Plugins?.AdoetzNative;

	const startNativeLiveVoice = async () => {
		const native = getAdoetzNative();
		if (!native?.startLiveVoice || nativeLiveVoiceStarted) return;

		try {
			await native.startLiveVoice({
			...
			nativeLiveVoiceStarted = true;
		} catch (error) {
			console.warn('Failed to start Android native live voice service:', error);
		}
	};

	const stopNativeLiveVoice = async () => {
		const native = getAdoetzNative();
		if (!native?.stopLiveVoice || !nativeLiveVoiceStarted) return;

		try {
			await native.stopLiveVoice();
		} catch (error) {
			console.warn('Failed to stop Android native live voice service:', error);
		} finally {
			nativeLiveVoiceStarted = false;
		}
	};

	const resumeLiveAudio = async () => {

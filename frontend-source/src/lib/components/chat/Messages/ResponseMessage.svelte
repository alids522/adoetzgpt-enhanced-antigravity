<script lang="ts">
	...
	import { estimateTokens, extractUsageTokenCounts, formatTokenCount } from '$lib/stores/tokenUsage';
	...
	$: usageCounts = extractUsageTokenCounts(
		message?.tokenUsage ?? message?.usage ?? message?.info?.usage ?? message?.info ?? {}
	);
	$: messageTokenCount =
		usageCounts.outputTokens > 0 ? usageCounts.outputTokens : estimateTokens(message?.content ?? '');
						...
						>
							...
							{#if messageTokenCount > 0}
								<div class="mt-1 text-[10px] font-medium text-gray-400 dark:text-gray-500">
									{formatTokenCount(messageTokenCount)} token
								</div>
							{/if}
						</div>
					</div>

<script lang="ts">
	...
	import { estimateTokens, extractUsageTokenCounts, recordTokenUsage } from '$lib/stores/tokenUsage';
	...
	onMount(() => {
		...
		// Listen for Gemini Live messages
		eventTarget.addEventListener('gemini:message', handleGeminiMessage as unknown as EventListener);
		eventTarget.addEventListener(
			'gemini:live-status',
			handleGeminiLiveStatus as unknown as EventListener
		);
		...
		return () => {
			try {
				...
				eventTarget.removeEventListener(
					'gemini:message',
					handleGeminiMessage as unknown as EventListener
				);
				eventTarget.removeEventListener(
					'gemini:live-status',
					handleGeminiLiveStatus as unknown as EventListener
	...
	const getTokenUsageModelMeta = (modelId = '') => {
		const model = $models.find((item) => item.id === modelId) as any;
		const normalizedModelId = (modelId || model?.id || '').toLowerCase();
		const provider =
			model?.owned_by ??
			(normalizedModelId.includes('gemini')
				...
				: normalizedModelId.includes('claude')
					...
					: model?.external
						...
						: 'unknown');
		return {
			model: modelId || model?.id || 'unknown',
			provider,
			endpoint:
				model?.source ??
				(model?.info as any)?.base_url ??
				(model?.info as any)?.baseUrl ??
				model?.owned_by ??
				provider
		};
	};

	const recordChatMessageTokenUsage = ({
		message,
		usage,
		modelId,
		source = 'chat'
		...
		if (!message || message.tokenUsageRecorded) return;
		const tokenHistory = history as any;
		const estimateMessageTokens = (item: any) =>
		...
		const inputTokens = message.parentId
			? createMessagesList(tokenHistory, message.parentId).reduce(
					(total: number, item: any) => total + estimateMessageTokens(item),
					0
				)
			: 0;
		const outputTokens = estimateMessageTokens(message);

		const counts = usage
			? extractUsageTokenCounts(usage)
			: {
					inputTokens,
					outputTokens,
		...
		if (!counts.totalTokens) return;

		const meta = getTokenUsageModelMeta(modelId ?? message.selectedModelId ?? message.model ?? '');
		recordTokenUsage({
			id: `${message.id}:${source}`,
			...counts,
			...meta,
			chatId: $chatId,
		...
		message.tokenUsageRecorded = true;
	};
	...
	const chatCompletionEventHandler = async (data, message, chatId) => {
		...
		if (done) {
			message.done = true;
			recordChatMessageTokenUsage({
				message,
				usage,
				modelId: selected_model_id ?? message.selectedModelId ?? message.model,
				source: 'chat'
			});
	...
	let activeGeminiLiveResponseId: string | null = null;
	let activeGeminiLiveParentId: string | null = null;
	let activeGeminiLiveSessionId: string | null = null;
	let lastGeminiLiveResponseId: string | null = null;
	let geminiLiveFinalizeTimer: ReturnType<typeof setTimeout> | null = null;
	let geminiLiveMessageQueue: Promise<void> = Promise.resolve();
	let geminiLiveTitleChatId: string | null = null;

	const getGeminiLiveModelMeta = (detail: any) => {
		const model = detail?.model ?? detail?.modelId ?? 'models/gemini-live';
		const modelName = detail?.modelDisplayName ?? model.replace(/^models\//, '');

		return { model, modelName };
	};

	const ensureLiveChatSaved = async () => {
	...
	const isDefaultChatTitle = () => {
		const title = ($chatTitle || '').trim();
		return !title || title === $i18n.t('New Chat') || title === 'New Chat';
	};

	const maybeGenerateGeminiLiveTitle = async (detail: any = {}) => {
		if ($temporaryChatEnabled || !$chatId || $chatId.startsWith('local:')) return;
		if (geminiLiveTitleChatId === $chatId || !isDefaultChatTitle()) return;
		if ($settings?.title?.auto === false) return;

		geminiLiveTitleChatId = $chatId;
		...
		const fallbackTitle =
			firstUserContent.length > 50 ? `${firstUserContent.slice(0, 50)}...` : firstUserContent;
		const model = selectedModels?.[0] ?? detail?.model ?? '';
		const generatedTitle = await generateTitle(localStorage.token, model, messages, $chatId).catch(
			(error) => {
				console.error('Failed to generate Gemini Live chat title:', error);
				return null;
			}
		);
		const title = generatedTitle || fallbackTitle;

		if (!title?.trim() || !isDefaultChatTitle()) return;
	...
	const resetGeminiLiveTurnState = () => {
		if (geminiLiveFinalizeTimer) {
			clearTimeout(geminiLiveFinalizeTimer);
			geminiLiveFinalizeTimer = null;
		}
		activeGeminiLiveResponseId = null;
		activeGeminiLiveParentId = null;
		lastGeminiLiveResponseId = null;
	};

	const clearGeminiLiveFinalizeTimer = () => {
		if (geminiLiveFinalizeTimer) {
			clearTimeout(geminiLiveFinalizeTimer);
			geminiLiveFinalizeTimer = null;
		}
	...
	const scheduleGeminiLiveFinalize = () => {
		clearGeminiLiveFinalizeTimer();
		geminiLiveFinalizeTimer = setTimeout(async () => {
			geminiLiveFinalizeTimer = null;
			await finalizeActiveGeminiLiveResponse();
			history = history;
		}, 8000);
	};

	const handleGeminiLiveStatus = async (event: CustomEvent) => {
		const detail = event.detail ?? {};
		const sessionId = detail.sessionId ?? null;

		if (detail.active) {
			activeGeminiLiveSessionId = sessionId;
			resetGeminiLiveTurnState();
			return;
		}

		if (!activeGeminiLiveSessionId || !sessionId || activeGeminiLiveSessionId === sessionId) {
			await finalizeActiveGeminiLiveResponse();
			resetGeminiLiveTurnState();
			if (!sessionId || activeGeminiLiveSessionId === sessionId) {
				activeGeminiLiveSessionId = null;
			}
		}
	};

	const finalizeActiveGeminiLiveResponse = async ({
		content = null,
		interrupted = false
	}: {
		content?: string | null;
		...
		if (!activeGeminiLiveResponseId) return null;

		const liveHistory = history as any;
		const message = liveHistory.messages[activeGeminiLiveResponseId];
		if (!message) {
			activeGeminiLiveResponseId = null;
			return null;
		}
		...
		if (interrupted && message.content?.trim() && !message.content.endsWith('[Interrupted]')) {
			message.content = `${message.content} [Interrupted]`;
		}

		if (!message.content?.trim() && interrupted) {
			if (message.parentId && liveHistory.messages[message.parentId]) {
				liveHistory.messages[message.parentId].childrenIds = liveHistory.messages[
					message.parentId
				].childrenIds.filter((id: string) => id !== message.id);
			}
			delete liveHistory.messages[message.id];
			history.currentId = message.parentId ?? null;
			activeGeminiLiveResponseId = null;
			await ensureLiveChatSaved();
			return null;
		}
		...
		liveHistory.messages[message.id] = message;
		...
		lastGeminiLiveResponseId = message.id;
		activeGeminiLiveResponseId = null;
	...
	const createGeminiLiveAssistantMessage = async (detail: any = {}) => {
		const liveHistory = history as any;
		const { model, modelName } = getGeminiLiveModelMeta(detail);
		const parentId = activeGeminiLiveParentId;

		if (!parentId) {
			return null;
		}

		if (activeGeminiLiveResponseId && liveHistory.messages[activeGeminiLiveResponseId]) {
			return liveHistory.messages[activeGeminiLiveResponseId];
		}

		const lastLiveResponse = lastGeminiLiveResponseId
			? liveHistory.messages[lastGeminiLiveResponseId]
			: null;
		if (
			lastLiveResponse &&
			lastLiveResponse.parentId === parentId &&
			lastLiveResponse.provider === 'gemini' &&
			lastLiveResponse.source === 'live' &&
			(!detail?.sessionId || lastLiveResponse.live?.sessionId === detail.sessionId)
		) {
			lastLiveResponse.done = false;
			lastLiveResponse.streaming = true;
			lastLiveResponse.live = {
				...(lastLiveResponse.live ?? {}),
				active: true
			};
			activeGeminiLiveResponseId = lastLiveResponse.id;
			liveHistory.messages[lastLiveResponse.id] = lastLiveResponse;
		...
		if (parentId && liveHistory.messages[parentId]) {
			liveHistory.messages[parentId].childrenIds = [
				...liveHistory.messages[parentId].childrenIds,
				responseMessageId
			];
		}

		liveHistory.messages[responseMessageId] = responseMessage;
		history.currentId = responseMessageId;
		activeGeminiLiveResponseId = responseMessageId;
	...
	// Gemini Live is treated as another streaming transport over the same chat history.
	const handleGeminiMessage = (event: CustomEvent) => {
		const detail = event.detail ?? {};
		geminiLiveMessageQueue = geminiLiveMessageQueue
			.catch((error) => {
				console.error('Gemini Live message queue error:', error);
			})
			.then(() => processGeminiMessage(detail));
	};

	const processGeminiMessage = async (detail: any = {}) => {
		const type = detail.type ?? (detail.role === 'user' ? 'user_final' : 'assistant_final');
		const content = detail.content ?? detail.text ?? '';
		const liveHistory = history as any;
		const sessionId = detail.sessionId ?? null;

		if (sessionId) {
			if (activeGeminiLiveSessionId && activeGeminiLiveSessionId !== sessionId) {
				return;
			}
			if (!activeGeminiLiveSessionId) {
				activeGeminiLiveSessionId = sessionId;
			}
		}
		...
		if (type === 'user_final') {
			...
			liveHistory.messages[userMessageId] = userMessage;
			history.currentId = userMessageId;
			activeGeminiLiveParentId = userMessageId;
			activeGeminiLiveResponseId = null;
			lastGeminiLiveResponseId = null;

			await ensureLiveChatSaved();
			void maybeGenerateGeminiLiveTitle(detail);
		}

		if (type === 'assistant_start') {
			clearGeminiLiveFinalizeTimer();
			if (!activeGeminiLiveResponseId) {
				await createGeminiLiveAssistantMessage(detail);
			}
		}
		...
		if (type === 'assistant_delta') {
			...
			const message = activeGeminiLiveResponseId
				? liveHistory.messages[activeGeminiLiveResponseId]
				: await createGeminiLiveAssistantMessage(detail);
		...
		if (type === 'assistant_final') {
			...
			if (activeGeminiLiveResponseId) {
				scheduleGeminiLiveFinalize();
			}
		}

		if (type === 'assistant_interrupted') {
			clearGeminiLiveFinalizeTimer();
			await finalizeActiveGeminiLiveResponse({ interrupted: true });
		}
